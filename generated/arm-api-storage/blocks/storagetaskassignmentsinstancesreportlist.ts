...
const StorageTaskAssignmentsInstancesReport_List: AppBlock = {
  name: "Storage Task Assignments Instances Report / List",
  description:
    "Fetch the report summary of all the storage task assignments and instances in an account",
  category: "Storage Task Assignments Instances Report",
  inputs: {
    default: {
      config: {
        accountName: {
      ...
      onEvent: async (input) => {
        const url =
          `https://management.azure.com/subscriptions/${input.event.inputConfig.subscriptionId || input.app.config.subscriptionId}/resourceGroups/${input.event.inputConfig.resourceGroupName || input.app.config.resourceGroupName}/providers/Microsoft.Storage/storageAccounts/${input.event.inputConfig.accountName}/reports` +
...
export default StorageTaskAssignmentsInstancesReport_List;
