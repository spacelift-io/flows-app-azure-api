...
const WebApps_StartWebSiteNetworkTraceOperation: AppBlock = {
  name: "Web Apps / Start Web Site Network Trace Operation",
  ...
  inputs: {
    default: {
      ...
      onEvent: async (input) => {
        const url =
          `https://management.azure.com/subscriptions/${input.event.inputConfig.subscriptionId || input.app.config.subscriptionId}/resourceGroups/${input.event.inputConfig.resourceGroupName || input.app.config.resourceGroupName}/providers/Microsoft.Web/sites/${input.event.inputConfig.name}/networkTrace/startOperation` +
...
export default WebApps_StartWebSiteNetworkTraceOperation;
