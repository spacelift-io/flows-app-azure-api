...
const Query_Execute: AppBlock = {
  name: "Query / Execute",
  description:
    "Executes an Analytics query for data. [Here](https://learn.microsoft.com/azure/azure-monitor/logs/api/request-format) is an example for using POST with an Analytics query.",
  category: "Query",
  inputs: {
    default: {
      config: {
      ...
      onEvent: async (input) => {
        ...
        const url = `https://${input.event.inputConfig.storageAccount || input.app.config.storageAccount}.blob.core.windows.net/workspaces/${input.event.inputConfig.workspaceId}/query`;
